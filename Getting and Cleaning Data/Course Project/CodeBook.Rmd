...
3. XYZ represent the x axis, y axis and z axis respectively


*The time domain signals (prefix 't') were captured at a constant rate of 50 Hz. Then they were filtered using a median filter and a 3rd order low pass Butterworth filter with a corner frequency of 20 Hz to remove noise.
...
1. Importing all the usefull files for train and test groups: 
                ...
                - train/X_train.txt or test/X_test.txt (while importing Set the names of the datasets columns to the features names)
                - subject_train.txt or test/subject_test.txt
2. Match the activity labels with the test and train labels (train/y_train.txt or test/y_test.txt)
3. Filter the columns to the features that correspond to mean or standard deviation of the measures
4. Merge the subject, activity labels and datasets together to one data frame (one for train data, other for test data)
5. Merge the test and train data
6. Group by activity label and participant
7. Calculate the mean for each variable for each activity and participan combination